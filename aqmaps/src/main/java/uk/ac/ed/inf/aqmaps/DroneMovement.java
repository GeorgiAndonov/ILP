...
public class DroneMovement {
	...
	private int movementAngle = 0;
	private Random dirDesider;
	private LineIntersector lineInt = new LineIntersector();
	
	public DroneMovement(String seed) {
		
		this.dirDesider = new Random(Long.parseLong(seed));
		
	}
	
	public void droneMovement(ArrayList<Line2D> confinementArea, double[] startingCoordinates, HashMap<double[], Station> route, ArrayList<ArrayList<Line2D>> noFlyZone, String day, String month, String year) throws FileNotFoundException {
		...
		while(counter < routeSize && moveCounter < 150) {
			...
			while(dist > 0.0002 || initMove == 0) {
				...
				this.setMovementAngle(CalculationFunctions.getAngle(currCoord, nextCoord));
				moveCounter++;
				
				// Calculate potential next coordinates
				var lngNew = currCoord[0] + Math.cos((this.getMovementAngle()*Math.PI) / 180)*0.0003;
				var latNew = currCoord[1] + Math.sin((this.getMovementAngle()*Math.PI) / 180)*0.0003;
				...
				// Test if the next coordinates leave the confinement area - chooses random location to continue
				while(this.getLineInt().isInConfinement(confinementArea, move)) {
					
					this.setMovementAngle(this.getDirDesider().nextInt(36) * 10);
					lngNew = prevCoord[0] + Math.cos((this.getMovementAngle()*Math.PI) / 180)*0.0003;
					latNew = prevCoord[1] + Math.sin((this.getMovementAngle()*Math.PI) / 180)*0.0003;
				...
				// Test if the next coordinates intersect a no fly polygon - take the angle of the line that was intersected to continue
				while(this.getLineInt().intersects(noFlyZone, move)) {
					
					coordChange1[0] = this.getLineInt().getIntersected().getX1();
					coordChange1[1] = this.getLineInt().getIntersected().getY1();
					
					coordChange2[0] = this.getLineInt().getIntersected().getX2();
					coordChange2[1] = this.getLineInt().getIntersected().getY2();
					
					// This takes into consideration if we have opposite angles
					this.setMovementAngle((CalculationFunctions.getAngle(coordChange1, coordChange2) + 180) % 360);
				...
				if(dist < 0.0002) {
					...
					logString += moveCounter + "," + prevCoord[0] + "," + prevCoord[1] + "," + this.getMovementAngle() + "," + currCoord[0] + "," + currCoord[1] + "," + route.get(nextCoord).getLocation() + "\n";
					
				} else {
					
					logString += moveCounter + "," + prevCoord[0] + "," + prevCoord[1] + "," + this.getMovementAngle() + "," + currCoord[0] + "," + currCoord[1] + "," + "null" + "\n";
		...
		// Add all unvisited stations to the map - if there are any
		if(counter != routeSize) {
        ...
        System.out.println("Final move counter = " + moveCounter);
	...
	public void setMovementAngle(int movementAngle) {
		this.movementAngle = movementAngle;
	}

	public Random getDirDesider() {
		return dirDesider;
	}

	public LineIntersector getLineInt() {
		return lineInt;
	}
